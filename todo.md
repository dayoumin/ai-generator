...
- [ ] Studio 화면을 점진적 공개(progressive disclosure) 방식으로 재구성한다.
  - 현재 작업 단계에 필요한 입력과 행동만 우선 노출한다.
  - 고급 실행 설정, 레퍼런스 관리, 배포 옵션은 관련 단계에서 펼치거나 요청할 때 보여준다.
  - 주요 흐름을 `프로젝트/소스 선택 → 생성 설정 → 실행 → 검수/배포`로 나누고 단계 전환 시 맥락을 유지한다.
  - 반복 사용자가 빠르게 접근할 수 있도록 마지막 설정 기억과 고급 패널 바로가기를 제공한다.
  - 변경 전후에 첫 생성까지의 선택 수, 화면 내 동시 노출 항목 수, 사용자 오류를 비교한다.
